...
* **현재 제공: Lo-Fi 만.** Hi-Fi 는 품질이 기준에 못 미쳐 숨김(`NEXT_PUBLIC_ENABLE_HIFI=1` 로 재노출). 코드·프롬프트는 유지.
* **Lo-Fi = 요소 완전성 우선 와이어프레임.** 스펙의 모든 컬럼·필드·액션·필터·상태·이동을 빠짐없이 그린다. 회색조, 자리표시자 데이터, 미정의 항목은 "[정렬 기준 미정의]" 같은 짧은 주석으로 표시. 화면당 170줄, 6,500토큰.
...
* 크기 상한: 화면당 170줄, 출력 7,000토큰, 화면 수 10개. 스펙의 컬럼·필드·액션은 개수와 무관하게 전부 표시.
* **페이지 섹션 처리(2026-09-22)**: 리포트·대시보드의 섹션(KPI·차트·요약 표·탭 내용)은 별도 화면으로 생성하되 LNB·흐름도에는 올리지 않고 부모 페이지 아래에 순서대로 이어 붙인다(`embed_in_parent`). 내용은 전부 보존되고 메뉴만 중복되지 않는다. (요약 카드로 접던 방식은 내용 대부분이 사라져 폐기)
