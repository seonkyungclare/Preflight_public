...
**분석 시간 단축 측정 (2026-09-21, 같은 샘플 PRD)**

| 구조 | 총 소요 | 출력 토큰 | 비고 |
| :--- | :--: | :--: | :--- |
| 단일 호출 (이전) | 269초 | 약 15,000~17,900 | Vercel 300초 제한에 근접 |
| A/B 병렬 + 출력 다이어트 + B Haiku | **66초** | A 4,387 + B 4,036 | A(Sonnet) 66초, B(Haiku 4.5) 30초. 결과 JSON 약 15KB |

점수는 43 → 54로 움직였다(§10 오픈 이슈 판정 변동). 출력이 짧아지면서 판정이 관대해질 수 있으니 실제 PRD 캘리브레이션 때 함께 본다.

**목업 생성 구조 변경 (2026-09-22)**
- 문제: 단일 함수 스트리밍 구조가 Vercel 300초를 두 차례 초과(15:47, 16:17). Pro 플랜 불가.
- 대응 1: Hi-Fi 를 "구조 충실 목업"으로 재정의(CLAUDE.md §A-8). 화면 5개 기준 85~102초 → 56초.
- 대응 2: 브라우저 지휘 3단계(spec → 화면별 screen 동시 → assemble). 화면당 함수 1개라 300초를 온전히 쓴다. 상세 모드(이전 프롬프트)를 토글로 복원.

**남은 후속 (이번 범위 밖)**
- 실제 PGT PRD 2~3건으로 점수 분포 캘리브레이션 (P5). 빈 템플릿 0~20점대, 예시 채운 템플릿 50~70점대가 목표.
- ~~분석 소요 시간이 약 200초로 길다.~~ → **2026-09-21 처리**: A(구조)/B(체크리스트) 병렬 호출 + 출력 다이어트 + B 에 빠른 모델 + 시스템 프롬프트 캐싱. 측정값은 아래 검증 기록 참조.
