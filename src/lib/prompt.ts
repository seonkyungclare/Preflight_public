...
개발자 항목에는 \`area\`로 **FE** 또는 **BE**를 넣으세요.

- **FE** — 화면 쪽에서 풀리는 것: 동작 순서, 입력 검증의 화면 반응, 사용자 조작에 대한 피드백,
  값을 감추거나 되돌릴 때의 화면 상태
- **BE** — 서버 쪽에서 풀리는 것: 데이터 규칙, 정합성, 외부 연동, 실패 시 데이터 상태, 권한,
  규모·성능, **배치·스케줄러·데이터 이관**

⚠️ **기준은 "그 공백이 어디서 풀리는가"이지 누가 구현하느냐가 아닙니다.**
배치·정산·이관·트랜잭션은 화면에서 보이더라도 **BE**입니다.

⚠️ **개수를 맞추려고 성격을 바꾸지 마세요.** 배치나 정산처럼 화면이 거의 없는 기능만
다루는 문서라면 BE만 나오는 것이 맞습니다.

다만 **아래는 놓치기 쉬운 FE 공백입니다.** 문서에 해당하는 기능이 있으면 반드시 찾아보세요.

- 조건에 따라 **필드를 감췄다 다시 보일 때** 입력값이 남는가 초기화되는가
- 사용자가 확인 버튼을 누른 뒤 **화면이 어떤 순서로 갱신되는가** (어디에 초점이 가는가)
- 입력 검증이 **언제 반응하는가** — 입력 중인가, 저장 시점인가
- 오래 걸리는 작업의 **진행 상태를 화면에서 어떻게 아는가** (폴링 주기, 완료 알림)
- 목록에서 **여러 건을 고를 때의 상한과 초과 시 화면 동작**

이런 공백이 있는데 BE로 몰아넣지 마세요. 찾아봐도 없으면 없는 대로 두세요.
...
이 도구의 값어치는 **선택지를 대신 만들어 주는 것**에 있습니다. "어떻게 하시겠어요?"는
PM도 이미 압니다. "[A]와 [B] 중 무엇인가요?"로 좁혀 줘야 결정이 빨라집니다.

- **binary**: 두 갈래로 갈림 → options 2개
- **multiple**: 3~4개 합리적 대안 → options 3~4개 (마지막에 "논의 필요" 허용)
- **open**: options ["논의 필요"]. **아래 셋 중 하나일 때만 씁니다.**
  - **수치** — 답이 값 하나다 (예: 오매핑률 목표치). 단, 구간으로 나뉘면 multiple로 쓰세요
  - **목록/스키마** — 항목을 열거해 채워야 한다 (예: 필드 목록, JSON 구조)
  - **매트릭스** — 조합별로 정의해야 한다 (예: 상태 3종 × 권한 조합)

⚠️ **open을 쓰기 전에 반드시 선택지를 먼저 시도하세요.** 위 셋에 해당하지 않으면
답은 대개 두세 갈래로 좁혀집니다. 특히 아래는 open이 아니라 binary/multiple입니다.

- 실패·중단 시 처리 → [전체 롤백] / [부분 유지 후 재시도] / [수동 처리]
- 충돌 시 우선순위 → 무엇을 우선할지의 갈래
- "A 때문인가 B 때문인가"처럼 **질문 안에 이미 갈래가 있는 것**

**open은 전체 질문의 3분의 1을 넘지 않아야 합니다.** 넘으면 좁힐 수 있는 것을
open으로 흘려보내고 있는 것입니다 — 다시 보세요.
...
4-1. open(논의 필요) 질문이 전체의 3분의 1을 넘지 않는가 —
     넘으면 수치·목록·매트릭스가 아닌 것을 open으로 흘려보낸 것이니 binary/multiple로 좁힐 것
5. area가 FE인 항목이 정말 화면 쪽에서 풀리는가 —
   배치·스케줄러·데이터 이관·정합성에 FE를 붙이지 않았는가
