...
export const DIMENSIONS: RubricDimension[] = [
  ...
  },
  {
    ...
    focus:
      ...
      '다만 몇 건을 나열했는지(개수)로 점수를 주지 마라 — 무엇이 미정인지 구체적인지, ' +
      '결정 주체와 기한이 함께 적혔는지로 판단한다. 대충 나열한 목록은 건수가 많아도 5~6점을 넘지 않는다. ' +
      '없는 정책을 지어 쓴 문서가 이 항목에서 이득을 보면 안 된다.',
    bands: [
      '9~10: 미결정 각각에 결정 주체와 기한이 구체적으로 적혔고, 본문 안에서도 [미정] 표기가 일관됨',
      '7~8: 미결정은 구체적으로 적혔으나 결정 주체나 기한이 일부 누락',
      '5~6: 미결정이 있다는 것은 밝혔으나 무엇이 미정인지 구체적이지 않거나 결정 주체·기한이 전부 빠짐 (개수가 많아도 이 구간)',
...
// ─── 개발 착수 전 확인 (점수에 반영하지 않음) ─────────────────────────────────
// 판정 라벨을 "디자인 착수 가능"으로 줄인 것의 짝이다.
// 문구만 줄이면 "그럼 개발 착수는 누가 확인하나"가 주인 없는 문제가 되므로,
// 축소한 만큼 눈에 보이게 한다.
//
// ⚠️ 왜 채점 축이 아닌가:
// ① 이 정보들은 링크된 별도 문서(API 설계서 등)에 정당하게 있을 수 있다
// ② 채점에 넣으면 화면·상태를 뺀 결정과 같은 논리를 거꾸로 되돌리는 범위 확장이 된다
// 그래서 있으면 확인 완료, 없으면 "확인 필요"로 보여주기만 한다.

// 있다/없다 두 갈래로는 "일부만 있는" 흔한 경우가 사라진다.
// 실측에서 확인: Wave.4는 네 항목 모두 '있음'으로 나왔지만 설명에는
// "…있으나 응답 시간 기대치는 없음"처럼 빠진 것이 적혀 있었다.
// 그러면 화면이 전부 초록으로 떠서 "이 문서만 봐서는 안 보인다"를 못 알린다.
export type DevReadinessStatus = '있음' | '부분' | '없음'

export interface DevReadinessCheck {
  key: string
  label: string
  focus: string // 프롬프트에 넣는 판정 기준
  why: string // 화면에 보여주는 "없으면 뭐가 막히나"
}

export function isDevReadinessStatus(v: unknown): v is DevReadinessStatus {
  return v === '있음' || v === '부분' || v === '없음'
}

export const DEV_READINESS: DevReadinessCheck[] = [
  {
    key: '데이터_규칙',
    label: '데이터 규칙',
    focus: '저장할 값의 형식·범위·필수 여부, 중복 판정 기준 같은 데이터 규칙이 서술되어 있는가',
    why: '없으면 서버가 무엇을 어떤 형태로 저장할지 정할 수 없습니다',
  },
  {
    key: '외부_연동',
    label: '외부 연동과 실패 처리',
    focus: '다른 시스템과 주고받는 것이 있는지, 있다면 그쪽이 실패했을 때 어떻게 할지가 있는가',
    why: '없으면 연동 상대가 죽었을 때의 동작이 개발자 임의로 정해집니다',
  },
  {
    key: '규모_성능',
    label: '규모·성능',
    focus: '한 번에 다루는 건수, 예상 사용량, 응답 시간 기대치 같은 규모 정보가 있는가',
    why: '없으면 동기로 처리할지 비동기로 뺄지 같은 구조 결정을 할 수 없습니다',
  },
  {
    key: '권한',
    label: '권한',
    focus: '누가 무엇을 할 수 있는지, 역할별 차이가 정의되어 있는가',
    why: '없으면 권한 검사를 어디에 걸지 정할 수 없습니다',
  },
]
